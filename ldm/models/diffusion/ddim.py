...
class DDIMSampler(object):
    def __init__(self, model, device, schedule="linear", **kwargs):
        ...
        self.device = device
        self.attribute_loss = AttrLoss(feat_ext='farl').eval().to(device)
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        # classifier guidance
        image = kwargs['image']
        target_image = kwargs['target_image']
        sim_weight = kwargs['sim_weight']
        dissim_weight = kwargs['dissim_weight']
        optim_weight = kwargs['optim_weight']

        grad = self.attr_opt(pred_x0, image, target_image,
                             sim_weight, dissim_weight, optim_weight)
        ...
        x_prev -= sigma_t * grad.float()
        return x_prev, pred_x0

    def attr_opt(self, x_0, image, target_image, sim_weight, dissim_weight, optim_weight):
        with torch.enable_grad():
            x_in = x_0.detach().requires_grad_(True)
            img = self.model.decode_first_stage(x_in)
            # self.show_intermidiate_img(img.clone())
            score_1 = self.attribute_loss(img, target_image) * sim_weight
            score_2 = self.attribute_loss(img, image) * dissim_weight
            return torch.autograd.grad(score_1-score_2, x_in)[0] * optim_weight
        
    def show_intermidiate_img(self, image_tensor):
